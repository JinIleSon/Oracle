...
ALTER TABLE treatment
    ADD CONSTRAINT treatment__un UNIQUE ( doc_id,
                                          pat_id,
                                          treat_no );
...
select d.doc_id, d.doc_name, d.doc_birth, d.dep_no, p.dep_name
        from department p join doctor d
        on p.dep_no = d.dep_no;
        
-- 문제2. 모든 간호사의 간호사ID, 이름, 생년월일, 전공의과 번호, 전공의과 이름을 조회하시오.
select n.nur_id, n.nur_name, n.nur_birth, n.dep_no, p.dep_name
        from department p join nurse n
        on p.dep_no = n.dep_no;
...
select 
        case
            when  substr(pat_jumin, 1, 2) <= substr(sysdate, 1, 2)
                 then substr(sysdate, 1, 2) - substr(pat_jumin, 1, 2)
            else substr(sysdate, 1, 4) - (1900+substr(pat_jumin, 1, 2))
        end "30세 이상 ~ 40세 미만 환자"
        from patient
        where substr(sysdate, 1, 4) - (1900+substr(pat_jumin, 1, 2)) between 30 and 39;

-- 문제8. 모든 전공부서에서 아직 의사ID가 부여되지 않은 의사의 이름과 부서명을 조회하시오.
select p.dep_manager, p.dep_name
        from doctor d right join department p
        on d.dep_no = p.dep_no
        where d.doc_id is null;

-- 문제9. 김태희 간호사가 맡고 있는 모든 환자의 이름을 구분자 ^으로 해서 '강동원^고소영^이나영'과 같이 조회하시오.
select listagg(p.pat_name, '^') within group(order by p.pat_name)
        from patient p join nurse n
        on p.nur_id = n.nur_id
        where n.nur_name = '김태희';

-- 문제10. 가장 많은 환자 수를 담당하는 간호사ID, 간호사 이름, 담당환자수를 조회하시오.
select n.nur_id, n.nur_name, count(*) as 담당환자수
        from nurse n join patient p
