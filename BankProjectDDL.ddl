...
select * from account;
-- Transaction
insert into transaction(trans_id, trans_acc_id, trans_type, trans_message, trans_money, trans_date) values (1, '1011-1001-1001', '입금', '2월 정기급여', 3500000, '2020-02-10 12:36:12');
insert into transaction(trans_id, trans_acc_id, trans_type, trans_message, trans_money, trans_date) values (2, '1011-1001-1003', '출금', 'ATM 출금', 300000, '2020-02-10 12:37:21');
insert into transaction(trans_id, trans_acc_id, trans_type, trans_message, trans_money, trans_date) values (3, '1011-1001-1002', '입금', '2월 급여', 2800000, '2020-02-10 12:38:21');
insert into transaction(trans_id, trans_acc_id, trans_type, trans_message, trans_money, trans_date) values (4, '1011-1001-1001', '출금', '2월 공과금', 116200, '2020-02-10 12:39:21');
insert into transaction(trans_id, trans_acc_id, trans_type, trans_message, trans_money, trans_date) values (5, '1011-1002-1005', '출금', 'ATM 출금', 50000, '2020-02-10 12:40:21');
...
select u.name as "고객의 이름", c.card_type as "카드종류"
        from customer u 
        join card c
        on u.cust_jumin = c.cust_jumin
        where c.card_limit >= 2000000;
        
-- 6) 예금계좌별 거래 건수 조회하시오.
select trans_acc_id as 예금계좌, count(*) as "예금계좌별 거래 건수" from transaction group by trans_acc_id;

-- 7) 거래금액이 100만원 이상인 거래 내역 조회하시오. (최근 거래순)
select * from transaction where trans_money >= 1000000 order by trans_date desc;
...
select sum(trans_money) as "예금구분이 '입금'인 거래의 총합" from transaction where trans_type='입금';
